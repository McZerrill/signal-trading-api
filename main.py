...
def analyze(symbol: str):
    try:
        ...
        ultima = hist_15m.index[-2].to_pydatetime().replace(second=0, microsecond=0, tzinfo=utc)
        close = round(hist_15m['close'].iloc[-2], 4)
        rsi = round(hist_15m['RSI'].iloc[-2], 2)
        ema9 = round(hist_15m['EMA_9'].iloc[-2], 2)
        ema21 = round(hist_15m['EMA_21'].iloc[-2], 2)
        ema100 = round(hist_15m['EMA_100'].iloc[-2], 2)
        atr = round(hist_15m['ATR'].iloc[-2], 2)
        macd = round(hist_15m['MACD'].iloc[-2], 4)
        macd_signal = round(hist_15m['MACD_SIGNAL'].iloc[-2], 4)
