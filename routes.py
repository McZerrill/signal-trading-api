# routes.py
# -----------------------------------------------------------------------------
# Endpoints FastAPI + monitor posizioni + hot assets + /price ottimizzato
# -----------------------------------------------------------------------------
...
def _active_sim_count() -> int:
    with _pos_lock:
        return sum(
            1
            for s in posizioni_attive.values()
    ...
    try:
        ...
        if spread > 5.0:
            try:
                ...
                try:
                    ...
                    if n >= 1:
                        ...
                        if n >= 3:
                            corpo_ref = (
                                (df_15m_tmp["close"] - df_15m_tmp["open"])
                                .iloc[:-1]
                                .abs()
                                .median()
                            )
                            volume_ref = (
                                df_15m_tmp["volume"].iloc[:-1].median()
                                if "volume" in df_15m_tmp.columns
                                else 0.0
                            )
                        elif n == 2:
                            p = df_15m_tmp.iloc[-2]
                        ...
                        COND_LISTING = (
                            float(u["close"]) / max(float(u["open"]), 1e-9) >= 2.0
                            and body_frac >= 0.70
                        )
                        COND_CORPO = corpo >= 3.0 * max(corpo_ref, 1e-9)
                        COND_VOLUME = (
                            "volume" in df_15m_tmp.columns
                            and float(u["volume"]) >= 2.5 * max(volume_ref, 1e-9)
                        )

                        trigger = (n == 1 and COND_LISTING) or (
                            n >= 2
                            and (
                                COND_LISTING
                                or (body_frac >= 0.70 and (COND_CORPO or COND_VOLUME))
                            )
                        )
                ...
                logging.warning(
                    f"⚠️ Errore nel recupero del prezzo per {symbol} con spread alto: {e}"
                )
                close_tmp = 0.0

            if close_tmp == 0.0:
                logging.warning(
                    f"⛔ Nessun prezzo disponibile per {symbol} (spread alto), risposta ignorata"
                )
                return SignalResponse(
                    symbol=symbol,
                    segnale="HOLD",
            ...
            return SignalResponse(
                ...
                segnale="HOLD",
                commento=(
                    f"Simulazione ignorata per {symbol} a causa di spread eccessivo.\n"
                    f"Spread: {spread:.2f}%"
                ),
                prezzo=close_tmp,
                take_profit=0.0,
        ...
        # 3) Gestione posizione già attiva
        if posizione:
            logging.info(
        ...
        logging.debug(
            f"[BINANCE] {symbol} – 15m: {len(df_15m)} | 1h: {len(df_1h)} | 1d: {len(df_1d)}"
        )
        ...
        # 1d: controllo RIGOROSO solo su EMA
        try:
            ...
            logging.debug(
                f"[1d STRICT] {symbol} – EMA7={e7d:.4f} EMA25={e25d:.4f} EMA99={e99d:.4f} -> {daily_state}"
            )
        ...
        if segnale in ["BUY", "SELL"]:
            ...
            logging.info(
                f"✅ Nuova simulazione {segnale} per {symbol} @ {close}$ – TP: {tp}, SL: {sl}, spread: {spread:.2f}%"
            )
...
# -----------------------------------------------------------------------------
# /price (usando helper con cache/timeout/retry)
# -----------------------------------------------------------------------------
@router.get("/price")
...
# Cache e log filtri (MANCAVANO → aggiunti qui)
_hot_cache = {"time": 0.0, "data": []}
_filtro_log = {
    "totali": 0,
    "atr": 0,
    "ema_flat": 0,
...
def hot_assets():
    ...
    for symbol in symbols:
        try:
            ...
            # Fast-path per LISTING PUMP con storico corto
            if 2 <= len(df) < 40:
                ultimo = df.iloc[-1]
                prev = df.iloc[-2]
                corpo = abs(float(ultimo["close"]) - float(ultimo["open"]))
                range_c = float(ultimo["high"]) - float(ultimo["low"])
                body_frac = corpo / max(range_c, 1e-9)

                if len(df) > 3:
                    corpo_ref = (
                        (df["close"] - df["open"]).iloc[:-1].abs().median()
                    )
                    volume_ref = df["volume"].iloc[:-1].median()
                else:
                    corpo_ref = abs(float(prev["close"]) - float(prev["open"]))
                    volume_ref = max(float(prev["volume"]), 1e-9)

                COND_GAIN = float(ultimo["close"]) / max(float(ultimo["open"]), 1e-9) >= 2.0
                COND_BODY = body_frac >= 0.70
                COND_CORPO = corpo >= 3.0 * max(corpo_ref, 1e-9)
                COND_VOLUME = float(ultimo["volume"]) >= 2.5 * max(volume_ref, 1e-9)
            ...
            volume_quote_medio = float(
                (df["volume"].tail(20) * df["close"].tail(20)).mean()
            )
            soglia_quote = 50_000 if MODALITA_TEST else 250_000
            ...
            ultimo = df.iloc[-1]
            corpo_candela = abs(float(ultimo["close"]) - float(ultimo["open"]))
            range_candela = float(ultimo["high"]) - float(ultimo["low"])
            corpo_medio = (
                (df["close"] - df["open"]).iloc[-21:-1].abs().mean()
            )
            volume_medio_20 = float(df["volume"].iloc[-21:-1].mean())

            upper_wick = float(ultimo["high"]) - max(
                float(ultimo["open"]), float(ultimo["close"])
            )
            lower_wick = min(float(ultimo["open"]), float(ultimo["close"])) - float(
                ultimo["low"]
            )
            ...
            cond_volume = float(df["volume"].iloc[-1]) > 2.0 * max(volume_medio_20, 1e-9)
            ...
            recenti_rialzo = all(
                df["EMA_7"].iloc[-i] > df["EMA_25"].iloc[-i] > df["EMA_99"].iloc[-i]
                for i in range(1, 4)
            )
            recenti_ribasso = all(
                df["EMA_7"].iloc[-i] < df["EMA_25"].iloc[-i] < df["EMA_99"].iloc[-i]
...
# -----------------------------------------------------------------------------
# Monitor posizioni attive (thread)
# -----------------------------------------------------------------------------
GESTIONE_ATTIVA = True
CHECK_INTERVAL_SEC = 60
TIMEFRAME_TREND = "15m"
CANDLE_LIMIT = 50
EMA_DIST_MIN_PERC = 0.0008
EMA_DIST_MAX_PERC = 0.0030
SL_BUFFER_PERC = 0.25
TP_TRAIL_FACTOR = 1.20
            ...
            try:
                ...
                try:
                    ...
                    with _pos_lock:
                        ...
                        new_sim = channel_defense_adjust(
                            sim, indicators=indicators, ch15=chan_15, log=sim["note_log"]
                        )
                ...
                trend_ok = (
                    (tipo == "BUY" and c >= ema7 and ema7 > ema25 and in_range)
                    or (tipo == "SELL" and c <= ema7 and ema7 < ema25 and in_range)
                )

                TICK = (
                    float(sim.get("tick_size", 0.0))
                    if isinstance(sim.get("tick_size", 0.0), (int, float))
                    else 0.0
                )
                eps = TICK
                ...
                if fill_price is not None:
                    with _pos_lock:
                        ...
                        sim["ora_chiusura"] = datetime.now(tz=utc).isoformat(
                            timespec="seconds"
                        )
                        if tipo == "BUY":
                        ...
                        sim["motivo"] = (
                            "🎯 TP colpito" if exit_reason == "TP" else "🛡️ SL colpito"
                        )
                ...
                if GESTIONE_ATTIVA and trend_ok and not verso_sl:
                    ...
                    if tipo == "BUY":
                        ...
                        if extra > 0:
                            nuovo_tp = max(
                                nuovo_tp,
                                round(tp + float(df["close"].iloc[-1]) * extra, 6),
                            )
